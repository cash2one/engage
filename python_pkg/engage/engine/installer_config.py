"""Parse installer configuration file (installer_config.json). This is the
...
class HostnameType(ConfigPropertyType):
    NAME = "hostname"
    def __init__(self):
        ConfigPropertyType.__init__(self, HostnameType.NAME)
    ...
    def get_default(self, default_base_value, config_choice_history):
        """If default is not specified, use hostname as default
        """
        if default_base_value!=None and default_base_value!="":
            return default_base_value
        else:
            return gethostname()
...
class LovType(ConfigPropertyType):
    NAME = "lov"
    def __init__(self, value_assoc_list):
        ConfigPropertyType.__init__(self, LovType.NAME)
        self.value_assoc_list = value_assoc_list
        self.user_input_options = [kv[0] for kv in value_assoc_list]

    def validate(self, data, config_choice_history=None):
        if data in self.user_input_options:
            return ValidationResults(True)
        else:
            return ValidationResults(False,
                       "Please specify one of %s" % self.user_input_options)

    def convert_to_json_type(self, data, config_choice_history):
        for kv in self.value_assoc_list:
            if kv[0] == data:
                return kv[1]
        assert 0, "No entry for %s found in LOV value mapping %s" % \
                  (data, self.value_assoc_list)
...
class ConfigProperty(object):
    def __init__(self, resource, name, typename, description, default=None, optional=None, datatype_args=None):
        self.resource = resource
        self.name = name
        ...
        if datatype_args:
            # the datatype may optionally take json argument(s)
            self.datatype_args = datatype_args
            self.type = type_constructor(datatype_args)
        else:
    ...
    def get_config_properties_for_install_spec(self, install_spec_choice_number):
        ...
        if spec.has_key(u"config_properties"):
            return [ConfigProperty(prop[u"resource"], prop[u"name"],
                                   ...
                                   prop[u"default"] if prop.has_key(u"default") else None,
                                   prop[u"optional"] if prop.has_key(u"optional") else None,
                                   prop[u"datatype_args"] if prop.has_key(u"datatype_args") else None)
    ...
    for config_prop in config_props:
        for key in [u"resource", u"name", u"type", u"description"]:
            if not config_prop.has_key(key):
                raise InstallerConfigError("genforma configuration file %s has invalid install config property definition: missing %s" %
                                          (filename, key))
        if not (config_prop[u"type"] in CONFIG_PROPERTY_TYPES):
            raise InstallerConfigError("genforma configuration file %s has invalid install config property definition: unkown property type %s" %
        ...
        for option in data[u"install_spec_options"]:
            for key in [u"choice_name", u"file_name"]:
                if not option.has_key(key):
                    raise InstallerConfigError("genforma configuration file %s has invalid install spec option: missing %s" %
                                              (filename, key))
...
def parse_installer_config(filename):
    if not os.path.exists(filename):
        raise InstallerConfigError("genForma configuration file %s does not exist" % filename)
    try:
        ...
        raise
    if type(data) != dict:
        raise InstallerConfigError("genForma configuration file %s data is not a dictionary" % filename)
    ...
    for (name, proptype) in required_properties:
        if not data.has_key(name):
            raise InstallerConfigError("genForma configuration file %s missing required property %s" %
                                      (filename, name))
        if type(data[name]) != proptype:
            raise InstallerConfigError("genForma configuration file %s property %s has wrong type: type was %s, expecting %s" %
    ...
    if data.has_key(u"application_archive") and data[u"application_archive"]!=None:
        ...
        for prop in app_archive_props:
            if not application_archive.has_key(prop):
                raise InstallerConfigError("Error in genForma configuration file %s: application_archive missing required subproperty %s" %
                                          (filename, prop))
        if application_archive[u"archive_type"] not in _app_archive_validator_map.keys():
            raise InstallerConfigError("Error in genForma configuration file %s: Unknown application achive type '%s'" %
                                      (filename, application_archive[u"archive_type"]))
    else:
        application_archive = None
                    
    return InstallerConfig(os.path.abspath(os.path.dirname(filename)), data[u"resource_def_file_name"],
