...
"""Resource manager for mongodb 2.4 
"""
...
class Manager(service_manager.Manager):
    ...
    def start(self):
        ...
        if soft<1000:
            self.ctx.logger.info("Setting soft limit of files to 1000, was %d" % soft)
            resource.setrlimit(resource.RLIMIT_NOFILE, (1000, hard))
        r(start_server,
          [p.server_exe, "--dbpath", p.dbpath,
           "--journal",
           "--port", str(p.config_port.port)],
          p.config_port.log_file,
          p.pid_file,
          cwd=os.path.dirname(p.server_exe),
          timeout_tries=20, time_between_tries=2.0,
          child_will_create_pid_file=True)
