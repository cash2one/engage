...
logger = engage.utils.log_setup.setup_script_logger("MySQL")
...
"""
    ...
    def __init__(self, package_name, id, home_path, port, admin_password,
        ...
        self.script_dir = os.path.join(self.home_path, "scripts")
...
def startup(config):
    rc = iuprocess.run_background_program(
             [os.path.join(config.bin_dir, "mysqld"),
...
class Manager(service_manager.Manager):
    ...
    def _setup_mysql_config_files(self):
        if not( os.path.exists(self.config.bin_dir)):
           # configure, make, and install mysql locally
    ...
    def install(self, package):
        ...
        if extracted_dir != self.config.home_dir:
            # make the final name of the mysql dir equivalent to the
