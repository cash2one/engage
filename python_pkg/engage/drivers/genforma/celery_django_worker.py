...
class Manager(django_worker_driver.Manager):
    ...
    def start(self):
        logger.info('Starting celeryd')
        self.start_worker('celeryd', [])
    def stop(self):
        self.stop_worker('celeryd')

    def force_stop(self):
        self.force_stop_worker('celeryd')

    def is_running(self):
        return self.is_running_worker('celeryd') 

    def get_pid_file_path(self):
        return os.path.join(self.config.input_ports.django.log_directory, 'celeryd.pid') 
